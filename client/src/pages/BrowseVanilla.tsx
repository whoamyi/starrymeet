...
import { CelebrityCard } from '@/components/browse';
...
export const BrowseVanilla = () => {
  ...
  // Fetch celebrities from real API
  ...
  const categories = useMemo(() => {
    const uniqueCategories = [...new Set(celebrities.map((c) => c.category).filter(Boolean))];
    return [
  ...
  const filteredCelebrities = useMemo(() => {
    return celebrities.filter((celeb) => {
      ...
      // Search filter
      if (searchQuery && !celeb.name.toLowerCase().includes(searchQuery.toLowerCase())) {
        return false;
      }

      // Price filter
      if (priceFilter !== 'all') {
        const price = celeb.price_per_hour || 0;
  ...
  }, [celebrities, selectedCategory, searchQuery, priceFilter]);
      ...
      <div className="toolbar" id="toolbar">
        ...
        <div className="toolbar-meta">
          <span id="resultsCount">
            {isLoading ? 'Loading...' : `${filteredCelebrities.length} results`}
          </span>
        ...
        <div className="swipe-section">
          <div className="card-container">
            <div className="card">
              {isLoading ? (
                ...
                <CelebrityCard celebrity={currentCard} viewMode="swipe" />
              ) : (
          ...
          <div className="controls">
            ...
            <button className="ctrl-btn view">
              <a href={currentCard ? `/celebrity/${currentCard.slug}` : '#'}>
        ...
        <div className="grid-section">
          <div className="grid">
            {isLoading ? (
              ...
              >
                <h3 style={{ color: '#fff', marginBottom: '12px' }}>No icons found</h3>
                <p>Try adjusting your filters</p>
              </div>
            ) : (
              paginatedCelebrities.map((celebrity) => (
                <CelebrityCard key={celebrity.id} celebrity={celebrity} viewMode="grid" />
              ))
            )}
          </div>

          {!isLoading && paginatedCelebrities.length > 0 && (
            <>
